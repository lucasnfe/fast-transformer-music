...
class VGMidiRealFake(torch.utils.data.Dataset):
    def __init__(self, pieces_path, seq_len, label, prefix=0):
        self.seq_len = seq_len
        self.pieces = self._load_pieces(pieces_path, seq_len, prefix)
        self.label = label
    ...
    def __getitem__(self, idx):
        x = self.pieces[idx]
        return torch.tensor(x), torch.tensor([self.label])
    ...
    def _load_pieces(self, pieces_path, seq_len, prefix_step=0):
        pieces = []
        for file_path in os.listdir(pieces_path):
            full_path = os.path.join(pieces_path, file_path)
            if os.path.isfile(full_path):
