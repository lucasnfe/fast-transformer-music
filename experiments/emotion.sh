...
DATA=../vgmidi
PRE_TRAINED=../trained/language_model_43.pth
SAVE_TO=../trained/
...
for i in `seq 0 $DATA_RANGE`; do
    ...
    $PYTHON $TRAINER --train $TRAIN_SET --test $TEST_SET --epochs $EPOCHS \
                     --seq_len $SEQ_LEN --vocab_size $VOCAB_SIZE --n_finetune $N_FINETUNE \
                     --pre_trained $PRE_TRAINED --n_layers $N_LAYERS --batch_size $BATCH_SIZE \
